# ai-service/app/services/enhanced_optimization.py - Enhanced 3-Stage AI Optimization Pipeline
import openai
import os
import json
import asyncio
...
from ..models.ai_models import AIProcessingSession, ResumeOptimization, AIServiceConfig
from ..database import get_db_context
from ..models.schemas import OperationType, ProcessingStatus, ConfidenceLevel
...
class EnhancedResumeOptimizer:
        ...
        try:
            with get_db_context() as db:
                ...
                for config in configs:
                    if config.config_key == "default_chat_model":
    ...
    async def create_processing_session(self, user_id: Optional[str] = None) -> str:
        """Create a new processing session for enhanced optimization"""
        session_id = f"enhanced_opt_{int(time.time())}_{hashlib.md5(str(time.time()).encode()).hexdigest()[:8]}"
        
        with get_db_context() as db:
            session = AIProcessingSession(
                session_id=session_id,
                user_id=user_id,
                operation_type=OperationType.ENHANCED_OPTIMIZATION.value,
    ...
    async def optimize_resume_complete(self, resume_text: str, job_description: str, user_context: Dict = None, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Complete 3-stage AI optimization pipeline
        Returns comprehensive optimization results
        """
        session_id = await self.create_processing_session(user_id)
        
        try:
            logger.info(f"Starting enhanced optimization pipeline for session {session_id}")
            
            # Stage 1: Deep Gap Analysis
            logger.info("🔍 Stage 1: Deep Gap Analysis...")
            analysis = await self.analyze_resume_gaps(resume_text, job_description)
            
            # Stage 2: Strategic Rewriting
            logger.info("✨ Stage 2: Strategic Rewriting...")
            ...
            # Stage 4: Enhancement Iteration (if needed)
            if validation.get("needs_improvement", False):
                logger.info("🔄 Stage 4: Enhancement Iteration...")
            ...
            # Store enhanced optimization in database
            await self._store_enhanced_optimization(session_id, resume_text, job_description, final_result)
            
            # Update session status
            await self.update_session_status(session_id, ProcessingStatus.COMPLETED)
            
            logger.info("🎉 Enhanced optimization pipeline completed successfully!")
            return final_result
            
        except Exception as e:
            await self.update_session_status(session_id, ProcessingStatus.FAILED, str(e))
            logger.error(f"Enhanced optimization failed: {e}")
            return await self.fallback_optimization(resume_text, job_description, session_id)
...
Make this resume absolutely exceptional."""
    ...
    async def _store_enhanced_optimization(self, session_id: str, resume_text: str, job_description: str, result: Dict):
        """Store enhanced optimization in database"""
        try:
            with get_db_context() as db:
                ...
                optimization_record = ResumeOptimization(
                    ...
                    improvements_made=result.get("optimization_details", {}).get("improvements_made", []),
                    keywords_added=result.get("optimization_details", {}).get("keywords_integrated", []),
                    ats_score_improvement=result.get("estimated_improvement", "+35%"),
                    match_score_prediction=0.85,  # Enhanced optimization typically has high match scores
                    confidence_score=result.get("confidence_score", 85.0),
                    confidence_level=self._get_confidence_level(result.get("confidence_score", 85.0)),
            ...
            logger.error(f"Failed to store enhanced optimization: {e}")
    ...
    async def fallback_optimization(self, resume_text: str, job_description: str, session_id: str) -> Dict[str, Any]:
